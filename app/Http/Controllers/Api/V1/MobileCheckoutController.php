...
final class MobileCheckoutController extends Controller
{
    public function flight(Request $request, TravelOffer $offer, FlightRevalidationService $revalidation, ExchangeRateService $rates, PaystackService $paystack): JsonResponse
    {
        ...
        try {
            ...
            if ($validation['price_changed'] ?? false) {
                return response()->json(['message' => 'The fare changed. Please review the updated total before paying.'], 409);
            }
    ...
    private function initialize(Request $request, PaystackService $paystack, string $currency, int $amount, string $email, array $context): JsonResponse
    {
        ...
        $attempt = CheckoutPaymentAttempt::create([
            'travel_offer_id' => $context['travel_offer_id'] ?? null, 'hotel_offer_id' => $context['hotel_offer_id'] ?? null,
            'session_fingerprint' => hash('sha256', $token.'|mobile'), 'client_token_hash' => hash('sha256', $token),
            'reference' => $reference, 'currency' => $currency, 'amount_minor' => $amount, 'email' => $email,
            'addon_ids' => [], 'checkout_payload' => $context['checkout_payload'],
        ]);
        $payment = $paystack->initialize($email, $amount, $currency, $reference, [
            'payment_attempt_id' => $attempt->id, 'booking_type' => $context['booking_type'], 'client_platform' => 'mobile',
        ]);
        $attempt->update(['access_code' => $payment['access_code'] ?? null]);

        return ApiResponse::success($request, [
            'attempt_id' => $attempt->id, 'reference' => $reference, 'client_token' => $token,
            'authorization_url' => $payment['authorization_url'] ?? null,
            'price' => ['currency' => $currency, 'total_minor' => $amount], 'status' => 'pending',
        ], status: 201);
    }
    ...
    private function completed(CheckoutPaymentAttempt $attempt): array
    {
        ...
        return ['status' => 'completed', 'reference' => $order->reference, 'order_id' => $order->id,
            'booking_type' => $booking?->product_type, 'provider_locator' => $booking?->provider_locator,

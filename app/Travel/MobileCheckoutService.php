...
final class MobileCheckoutService
{
    ...
    public function complete(CheckoutPaymentAttempt $attempt, array $gateway): Order
    {
        ...
        return DB::transaction(function () use ($attempt, $gateway): Order {
            ...
            Payment::create([
                'order_id' => $order->id, 'gateway' => 'paystack', 'gateway_reference' => $attempt->reference,
                'status' => 'paid', 'currency' => $attempt->currency, 'amount_minor' => $attempt->amount_minor, 'paid_at' => now(),
    ...
    private function customer(array $data): Customer
    {
        ...
        $customer->fill([
            ...
            'phone' => $data['phone'] ?? '', 'title' => $data['title'] ?? null, 'date_of_birth' => $data['date_of_birth'] ?? null,
            'gender' => $data['gender'] ?? null, 'nationality' => $data['nationality'] ?? null,
            'passport_number' => $data['passport_number'] ?? null, 'passport_country' => $data['passport_country'] ?? null,
            'passport_expires_at' => $data['passport_expiry'] ?? null, 'status' => 'active',
        ])->save();

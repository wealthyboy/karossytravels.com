...
@php
    ...
    $money = fn (int $minor) => ($currency === 'NGN' ? '₦' : '$').number_format($minor / 100, 2);
...
<section class="booking-page"><div class="container public-container">
    ...
    <div class="row g-4"><div class="col-lg-8">
        ...
        <div class="booking-card mt-4"><h2 class="section-title mt-0">Fare conditions</h2><ul class="mb-0"><li>Taxes and configured Karossy markup are included.</li><li>{{ ($fare['refundable'] ?? false) ? 'Refundable fare' : 'Cancellation and change fees may apply' }}.</li><li>The fare will be revalidated before booking.</li></ul></div>
    </div><aside class="col-lg-4"><div class="checkout-summary sticky-lg-top"><h2>Price summary</h2><div><span>Base fare and taxes</span><strong>{{ $money($provider['amount_minor']) }}</strong></div><div><span>Karossy markup</span><strong>{{ $money($markup['amount_minor']) }}</strong></div><div class="summary-total"><span>Total</span><strong>{{ $money($total['amount_minor']) }}</strong></div><small>Displayed in {{ $currency }}. Final availability is confirmed before booking.</small><button class="btn btn-karossy w-100" type="button" data-bs-toggle="modal" data-bs-target="#publicFlightRevalidationModal" data-public-revalidate data-url="{{ route('flights.offers.revalidate', $offer) }}">Continue to travellers <i class="bi bi-arrow-right"></i></button><p><i class="bi bi-shield-check"></i> Secure booking with Karossy support</p></div></aside></div>
</div></section>

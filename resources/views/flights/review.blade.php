...
<section class="booking-page"><div class="container public-container">
    ...
    </div><aside class="col-lg-4"><div class="checkout-summary sticky-lg-top"><h2>Price summary</h2><div><span>Flight, taxes and fees</span><strong>{{ $money($provider['amount_minor']) }}</strong></div>@if($markup['amount_minor'] > 0)<div><span>Service markup</span><strong>{{ $money($markup['amount_minor']) }}</strong></div>@endif @if($discount['amount_minor'] > 0)<div class="text-success"><span>Flight deal discount</span><strong>−{{ $money($discount['amount_minor']) }}</strong></div>@endif<div class="summary-total"><span>Total</span><strong>{{ $money($total['amount_minor']) }}</strong></div><small>Displayed in {{ $currency }}. Final availability is confirmed before booking.</small><button class="btn btn-karossy w-100" type="button" data-bs-toggle="modal" data-bs-target="#publicFlightRevalidationModal" data-public-revalidate data-url="{{ route('flights.offers.revalidate', $offer) }}">Continue to travellers <i class="bi bi-arrow-right"></i></button><p><i class="bi bi-shield-check"></i> Secure booking with Karossy support</p></div></aside></div>
</div></section>

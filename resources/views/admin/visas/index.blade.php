...
<header class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
    <div>
        ...
        <p class="text-secondary mb-0">Manage visa destinations, processing durations, requirements and customer fees.</p>
    </div>
...
<div class="card content-card admin-table-card">
    <div class="table-responsive">
        <table class="table admin-data-table align-middle mb-0">
            <thead><tr><th class="ps-4">Country</th><th>Duration</th><th>Customer fee</th><th>Status</th><th class="text-end pe-4">Actions</th></tr></thead>
            <tbody>
            @forelse($visas as $visa)
                <tr>
                    <td class="ps-4"><strong>{{ $visa->country }}</strong></td>
                    <td>{{ number_format($visa->duration_days) }} days</td>
                    <td>NGN {{ number_format($visa->fee_cents / 100, 2) }}</td>
            ...
            @empty
                <tr><td colspan="5" class="text-center text-secondary py-5"><i class="bi bi-passport d-block fs-2 mb-2"></i>No visa services have been added.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>

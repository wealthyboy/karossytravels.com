...
<section class="booking-page"><div class="container public-container">
    ...
    <form class="row g-4" method="POST" action="{{ route('hotels.checkout.payment', $offer) }}" data-verify-url="{{ route('hotels.checkout.verify', $offer) }}" @if($recoverableAttempt) data-recovery-reference="{{ $recoverableAttempt->reference }}" @endif data-hotel-checkout>
        ...
        <div class="col-lg-8">
            ...
            <div class="checkout-actions"><a class="btn btn-outline-dark" href="{{ route('hotels.rooms', $offer) }}"><i class="bi bi-arrow-left"></i> Back to rooms</a><button class="btn btn-karossy checkout-pay-button" type="submit" data-hotel-pay>Pay {{ $money($total['amount_minor']) }} <i class="bi bi-arrow-right"></i></button></div>
        </div>

...
@section('content')
@php
    $stay = data_get($booking->details, 'stay', []);
...
<section class="booking-page checkout-complete-page"><div class="container public-container">
    ...
    <div class="confirmation-hero"><span class="completion-icon"><i class="bi bi-check-lg"></i></span><div><span class="public-eyebrow">Reservation received</span><h1>Your hotel booking is complete</h1><p>Your booking confirmation has been sent to <strong>{{ data_get($order->customer, 'email') }}</strong>.</p></div><span class="confirmation-status"><i class="bi bi-check-circle-fill"></i> Paid</span></div>
    ...
    <aside class="col-lg-4"><div class="confirmation-panel confirmation-summary"><span class="public-eyebrow">Booking reference</span><h2>{{ $order->reference }}</h2><strong class="confirmation-total">{{ \App\Support\CurrencyMetadata::format($order->total_minor, $order->currency) }}</strong><div class="confirmation-contact"><i class="bi bi-envelope-check"></i><span><small>Receipt sent to</small><strong>{{ $receiptEmail }}</strong></span></div><a class="btn btn-karossy w-100" href="{{ route('account.bookings.show', $booking) }}">View booking</a><a class="btn btn-outline-dark w-100" href="{{ route('home') }}">Back to home</a></div></aside></div>
</div></section>
@endsection
